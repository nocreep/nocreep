...
const retentionInstruction =
  "Never prune task context, judgments, or the basis for your reasoning. Keep them available even after you have already reasoned, acted, or decided based on them; they remain required context unless they are completely unrelated to the active task. But strip not-needed lines (irrelevant, no information, ...) from needed outputs."
const toolUseInstruction =
  "Group independent searches, globs, greps, and reads into the same parallel tool-call batch whenever possible, including alongside nocreep. Prefer one appropriately sized read with enough surrounding context over several tiny read slices. Still be mindful of tokens but not overly conservative, just prune unneeded output lines with nocreep instead of making repeated narrow reads."
    ...
    nocreep: tool({
      ...
      args: {
        indices: tool.schema
          ...
          .min(1)
          .describe(
            "0-based indices of completed tool calls from the immediately preceding tool batch to prune (or partially prune). You will not be able to see this output (or the lines of it) again, if you needed them for your reasoning or next step, keep them. 0 means the first tool call in that batch by position (order started, not order finished).",
          ),
        lines: tool.schema
          .array(tool.schema.array(tool.schema.union([tool.schema.number().int().min(1), tool.schema.string()])))
          .optional()
          .describe(
            'Optional nested 1-based output lines or ranges to remove, one array per index, for example [[5, "10-25"], []]. Omit lines, pass [], or pass [] for an index to prune the whole output. Try to not prune complete outputs unless it failed in a non-fixable unrelated way or is completely irrelevant.',
          ),
      },
